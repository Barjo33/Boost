...
- **Signing/secrets:** `keystore.properties` was TRACKED + pushed with real signing passwords (store/keyPassword=[REDACTED-see-local-keystore.properties]) + an absolute storeFile path (broke third-party builds). Now `git rm --cached`'d (still on disk locally for Tim's builds, untracked) + `keystore.properties.template` added. build.gradle already guards signingConfig on the file existing → fresh clones build unsigned cleanly. **DO NOT re-add keystore.properties to git.** It was committed long ago (commit ad9d7fe175, pre-this-session), not introduced by this session.
