...
**AGGRESSION relation (2026-07-11 code check):** raising Boost aggression makes CONFIRMED shots BOTH bigger AND earlier — `mealActionMultiplier`=base×aggression (CONFIRMED only, base 1.8), and the OBSERVING→CONFIRMED **dose-adequacy gate** (`prospectiveConfirmShot = budget×1.8×aggression×velocity > ~fixed floor`, DetermineBasalBoostV5.kt:208-212,247) lets higher aggression clear the floor sooner. BUT the agreed crash driver here is **late+large** (big shot's ~3h SMB action outlasts the self-clearing fast carb → into the fall); eager-confirm context was the DISCRIMINATOR, not the cause. So aggression's dominant fast-carb harm is SIZE; don't raise it; fix stays V7 (sizes down). Don't over-attribute the timing angle.
