 ...
 *
 * Ported from the tsunami `devTsuV35_1` AdaptiveSmoothingPlugin to the 3.4 (Boost) codebase.
 * The UKF mathematics (predict/update/sigma-points/matrix-sqrt/R-adaptation) are verbatim.
 * Deliberate adaptations for this codebase, all documented inline:
 *  - Implements the non-suspend single-arg `Smoothing.smooth(data)` (no SmoothingContext).
 *  - IOB (used only by the compression-artifact guard) is fetched via [IobCobCalculator]
 *    behind a `dagger.Lazy` (breaks a construction cycle) and FAILS SAFE to a high value
 *    (compression rejection disabled → real drops are never masked).
 *  - `learnedR` is kept in-memory (re-converges in ~48 readings) instead of persisted via
 *    the v4-only `DoubleNonKey`; the >24h-gap learning reset is retained.
 *  - The reactive sensor-change subscription (config/coroutines/observeChanges) is dropped;
 *    the gap reset covers the new-sensor case.
 */
@Singleton
class AdaptiveSmoothingPlugin @Inject constructor(
    ...
    // Processing state (in-memory; re-converges after a gap/restart)
    private var learnedR = DEFAULT_LEARNED_R
    ...
    override fun smooth(data: MutableList<InMemoryGlucoseValue>): MutableList<InMemoryGlucoseValue> {
        ...
        try {
            ...
            processHybridSegment(data, currentIobTotalU())

            sanitizeOutput(data)
            return data

        } catch (e: Exception) {
            aapsLogger.error(LTag.GLUCOSE, "AdaptiveSmoothing: Error, falling back to raw", e)
            copyRawToSmoothed(data)
            sanitizeOutput(data)
            return data
        }
    ...
    /**
     * Total IOB (units) for the compression-artifact guard.
     * FAILS SAFE: on any error we return a HIGH value so compression rejection is disabled
     * and genuine (insulin-driven) drops are never held/masked.
     */
    private fun currentIobTotalU(): Double =
        try {
            ...
            aapsLogger.debug(LTag.GLUCOSE, "AdaptiveSmoothing: IOB unavailable, disabling compression guard")
            IOB_SAFE_FALLBACK_U
        }
    ...
    private fun processHybridSegment(
        ...
        for (i in startIdx downTo 0) {
            ...
            if (isRapidManeuver) {
                aapsLogger.debug(LTag.GLUCOSE, "AdaptiveSmoothing: RAPID RISE DETECTED (Innov=${preFitInnovation.toInt()}). Inflating Q for Zero-Lag.")
            ...
            if (isCompression) {
                aapsLogger.warn(LTag.GLUCOSE, "AdaptiveSmoothing: COMPRESSION BLOCKED at ${z.toInt()} mg/dL. Holding prediction.")
                ...
                if (isKineticHypo) {
                    ...
                    aapsLogger.debug(LTag.GLUCOSE, "AdaptiveSmoothing: KINETIC HYPO DETECTED! Vel=$velocity, Pred20=$predictedBg20min. Forcing low.")
    ...
    private fun shouldResetLearning(currentTimestamp: Long): Boolean {
        ...
        if (diffMinutes > 1440) return true // >24h gap → likely new sensor / long outage
        return false
    }

    private fun resetLearning() {
        learnedR = DEFAULT_LEARNED_R
        innovations.clear()
        rawInnovationVariance.clear()
        aapsLogger.info(LTag.GLUCOSE, "AdaptiveSmoothing: Learning Reset. R=$learnedR")
    }
    ...
    private companion object {
        ...
        // Default measurement-noise variance (~6 mg/dL sensor std). Re-learned online.
        const val DEFAULT_LEARNED_R: Double = 36.0

        // High sentinel so the compression guard fails safe (disabled) when IOB is unavailable.
        const val IOB_SAFE_FALLBACK_U: Double = 99.0
    }
}
