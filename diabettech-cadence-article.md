...
The glucose those three copies observe responds to the insulin my pump delivered rather than to the insulin they believe they delivered, so their internal state is self consistent but counterfactual. I assumed at first that this would compound and planned to reset them daily, and that was wrong. Insulin decays, so the gap between an instance's insulin on board and reality settles at roughly the difference in dosing rate multiplied by the mean time a unit hangs around, which for my curve is about seventy five minutes. An instance dosing ten per cent more than reality is carrying about 0.05 U that does not exist, against the 1.6 U I typically have on board. It does not grow with the length of the run, so the copies are left to run without resetting. The offset is recorded and reported anyway, because that reasoning is a prediction and it might be wrong.
