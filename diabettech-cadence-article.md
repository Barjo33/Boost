...
The glucose those three copies observe responds to the insulin my pump delivered rather than to the insulin they believe they delivered, so their internal state is self consistent but counterfactual. The gap does not grow without limit, because insulin decays. It settles at roughly the difference in dosing rate multiplied by the mean time a unit hangs around, which for my curve is about seventy five minutes, so a copy dosing ten per cent more than reality is carrying about 0.05 U that does not exist against the 1.6 U I typically have on board. The copies therefore run without resetting. I record the offset and report it anyway, because that is a prediction about how they will behave and it might not hold.
