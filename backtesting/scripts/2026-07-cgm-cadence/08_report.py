...
Ac = L.read("06_acceleration.json")
...
w("")
w("## 7. Point to point acceleration")
w("")
w("Velocity is the first difference of consecutive readings and acceleration the difference of")
w("consecutive velocities, so over one sampling interval h the literal construction is")
w("(x(t) - 2x(t-h) + x(t-2h)) / h^2. Figures below are in mg/dL per 5 min per 5 min so that the")
w("two cadences can be read against each other.")
w("")
w("| | Five-minute era | One-minute era |")
w("|---|---|---|")
w(f"| Samples | {Ac['e5']['n']:,} | {Ac['e1']['n']:,} |")
w(f"| SD over one interval (mg/dL) | {Ac['e5']['sd_raw_mgdl']:.2f} | {Ac['e1']['sd_raw_mgdl']:.2f} |")
w(f"| SD as mg/dL per 5 min per 5 min | {Ac['e5']['sd_per5per5']:.2f} | {Ac['e1']['sd_per5per5']:.2f} |")
w(f"| Median absolute value (mg/dL) | {Ac['e5']['median_abs_raw']:.2f} | {Ac['e1']['median_abs_raw']:.2f} |")
w(f"| Exactly zero | {Ac['e5']['pct_zero_raw']:.1f}% | {Ac['e1']['pct_zero_raw']:.1f}% |")
w(f"| Predicted from the variogram (mg/dL) | {Ac['e5']['variogram_pred_sd_raw']:.2f} | {Ac['e1']['variogram_pred_sd_raw']:.2f} |")
w(f"| Lag-1 autocorrelation | {Ac['e5']['acf1']:+.3f} | {Ac['e1']['acf1']:+.3f} |")
w("")
w("The mean square of a second difference is exactly 4D(h) minus D(2h) for any process, so the")
w("variogram of section 3 predicts the acceleration magnitude with no free parameters. On the")
w(f"five-minute record the prediction is {Ac['e5']['variogram_pred_sd_raw']:.2f} mg/dL against a")
w(f"measured {Ac['e5']['sd_raw_mgdl']:.2f}. Acceleration therefore carries nothing the variogram")
w("did not already describe.")
w("")
w("### 7.1 It has no cadence-independent value")
w("")
sd_ = Ac["scale_dependence"]
w("For a process whose variogram goes as h to the power alpha, the second difference goes as the")
w("same power, so acceleration, which divides by h squared, goes as h to the power alpha/2 minus")
w("2. The number therefore depends on the interval it was computed over.")
w("")
w("| Ratio of one-minute to five-minute acceleration SD | Value |")
w("|---|---|")
w(f"| Measured | {sd_['measured_ratio']:.2f} |")
for lab, v in sd_["by_alpha"].items():
    if v["alpha"] is None:
        w(f"| A twice differentiable signal | {v['predicted_ratio']:.2f} |")
    else:
        w(f"| Predicted from alpha = {v['alpha']:.2f} ({lab}) | {v['predicted_ratio']:.2f} |")
w("")
w("A twice differentiable signal would give a ratio of 1.00, because the leading term cancels")
w("and acceleration converges on a real value as the interval shrinks. This record gives 7.65,")
w("close to the value implied by its own roughness. Acceleration measured over one minute is")
w("roughly eight times larger than the same quantity measured over five minutes, and neither is")
w("more correct than the other.")
w("")
w("The practical consequence is that any threshold placed on a point to point acceleration is a")
w("threshold at one particular cadence and does not transfer to another. The construction used")
w("by the controller avoids this, since it averages rates over windows fixed in minutes rather")
w("than in samples, and normalises by the slower of the two.")
w("")
w("### 7.2 It adds nothing to prediction")
w("")
w("Adding acceleration to a feature set that already contains the current value and several")
w("backward differences and slopes, then predicting within 30 minutes:")
w("")
w("| Era | Task | Feature set | AUC | Lift |")
w("|---|---|---|---|---|")
for k, lab in (("e5", "5-min"), ("e1", "1-min")):
    for task, rows in Ac["predictive_value"][k].items():
        for sname, r in rows.items():
            w(f"| {lab} | {task.replace('<', 'below ').replace('>', 'above ')} | {sname} | "
              f"{r['auc']:.4f} [{r['lo']:.4f}, {r['hi']:.4f}] | {r['lift']:.2f}x |")
w("")
w("Every variant sits within a few thousandths of the velocity-only baseline and every interval")
w("overlaps it. Neither the point to point form nor the controller form carries predictive")
w("information that the velocity terms do not already hold, at either cadence.")
w("")
w("## 8. What cadence does change")
w("")
