...
cur.execute("""select ts_epoch, cgm_mgdl, variable_sens, sug_current_target, iob_iob,
   ...
   boostv5_confirmedcap, boostv5_committedcap
...
A = {k: np.array([r[i] for r in rows], dtype=object) for i, k in enumerate(
    ['e', 'bg', 'isf', 'tgt', 'iob', 'state', 'fd', 'budget', 'knob', 'ccap', 'mcap'])}
...
for k in range(len(rows)):
    ...
    for m in range(0, HORIZON + 1, 5):
        ...
        bf = ba + pert
        bg_act_path.append(ba); bg_fix_path.append(bf)
    ...
    ba = np.array(bg_act_path); bf = np.array(bg_fix_path)
    nadir_a, nadir_f = ba.min(), bf.min()
    plat_a = ba[-6:].mean(); plat_f = bf[-6:].mean()
    meals.append(dict(user=USER, rise=rise, iob0=iob0, s_v6=round(s_v6, 3), s_fix=round(s_fix, 3),
                      ...
                      nadir_a=round(nadir_a, 1), nadir_f=round(nadir_f, 1),
                      plat_a=round(plat_a, 1), plat_f=round(plat_f, 1),
                      crash_a=int(nadir_a < 70), crash_f=int(nadir_f < 70),
                      deep_a=int(nadir_a < 54), deep_f=int(nadir_f < 54),
                      plateau_high_f=int(plat_f > 160)))
