...
2. **Activity owns nearly half of all low-time (47%), rescue-overshoot another 37% — stacking is only 16%.** Boost's *dosing* is not the main low driver; exercise and rescue dynamics are. This validates the exercise protections and the Garmin HR/steps ingest as the **#1 low lever**, and points at rescue handling second.
...
2. **Activity → hypo** — 47% of low-time. The Garmin ingest + exercise protections are the top low lever; worth confirming activity genuinely *precedes* the fall (a small predictive check).
...
- **Decision-tree ordering** puts BRAKE before CAP_CLIP/UNDERSIZED, so a cycle that is both braked and clipped counts as brake — this inflates upstream causes; the ordering is a documented judgement call and can be swept.
