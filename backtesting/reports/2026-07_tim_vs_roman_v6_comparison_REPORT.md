...
The era definition was corrected from `boostv5_state IS NOT NULL` (tim's whole boost-other history,
~62 d back to 05-07, **mostly shadow**) to **`boostv5_active = true`** (V6 actually drove the pump).
Corrected windows: **tim V6-active from 2026-06-26, 2961 cycles / 12 d**; **H from 06-30, 2011 / 8 d**.
The earlier "tim 62 d / 10.8 U-day / mostly-shadow" figures are void. Corrected aggregates below.
...
sometimes large. (The 3.75 fresh confirms/day is fewer than tim's 5.67, but each is a real shot.)
...
2. **Of the holds that DO want to dose** (budget median 1.73U), the single dominant everyday cut is the
   **committedCap clamp — every clamped hold bound at cap = 0.5U** (budget 1.73 → delivered 0.50), 36% of
   all suppressed COMMITTED-U. Then iobHeadroomBrake (29%, IOB rising through the meal) and velocityFactor
   (17%, his rises are slow/plateau → the 0.4× floor).

### THE one dominant everyday reason
**The committedCap = 0.5** clamping holds that want 1.4–3.3U down to 0.5. Note 0.5 is the *factory-default*
...
- Physiology differs (tim TDD ~13 / ISF ~130 vs H TDD ~50 / ISF ~36) — compare shapes, not raw U.
